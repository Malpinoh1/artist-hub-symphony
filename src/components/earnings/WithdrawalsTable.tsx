          ...
          <React.Fragment key={step.key}>
            <TooltipProvider>
              <Tooltip>
                ...
                <TooltipContent><p>{step.label}</p></TooltipContent>
              </Tooltip>
            </TooltipProvider>
            {index < steps.length - 1 && (
              <div className={`w-4 h-0.5 ${index < currentIndex ? 'bg-green-500' : 'bg-muted'}`} />
            )}
          </React.Fragment>
        );
      })}
...
const WithdrawalsTable: React.FC<WithdrawalsTableProps> = ({ withdrawals }) => {
  ...
  const activeWithdrawal = withdrawals.find(w => ['PENDING', 'APPROVED', 'PROCESSING'].includes(w.status));
          ...
          <div key={withdrawal.id} className="bg-card border rounded-lg p-4 space-y-3">
            <div className="flex justify-between items-start">
              <div>
                ...
                {(withdrawal.credit_deduction || 0) > 0 && (
                  <p className="text-xs text-amber-600 dark:text-amber-400">
                    Credit deducted: -${(withdrawal.credit_deduction || 0).toLocaleString()}
                  </p>
                )}
                <p className="text-sm text-muted-foreground">
                  Payout: ${(withdrawal.final_amount ?? withdrawal.amount).toLocaleString()} (₦{((withdrawal.final_amount ?? withdrawal.amount) * EXCHANGE_RATE).toLocaleString()})
                </p>
              </div>
            ...
            <div className="text-sm">
              ...
              {withdrawal.bank_name && <p className="text-muted-foreground">{withdrawal.bank_name}</p>}
            </div>
          ...
          <TableBody>
            {withdrawals.map((withdrawal) => (
              <TableRow key={withdrawal.id}>
                ...
                <TableCell className="whitespace-nowrap">
                  {(withdrawal.credit_deduction || 0) > 0 ? (
                    <span className="text-amber-600 dark:text-amber-400">-${(withdrawal.credit_deduction || 0).toLocaleString()}</span>
                  ) : '—'}
                </TableCell>
                <TableCell className="whitespace-nowrap font-medium">
                  ${(withdrawal.final_amount ?? withdrawal.amount).toLocaleString()}
                  <span className="text-xs text-muted-foreground ml-1">
                    (₦{((withdrawal.final_amount ?? withdrawal.amount) * EXCHANGE_RATE).toLocaleString()})
                  </span>
                </TableCell>
                <TableCell className="whitespace-nowrap">
